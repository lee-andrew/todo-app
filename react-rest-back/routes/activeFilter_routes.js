...
const fs = require('fs');
const filename = "./data/activeFilterDB.txt";

router.get('/activeFilter', function(req, res) {
    fs.readFile(filename, 'utf8', function(err, data) {
        ...
        if ( !data ) {
            res.send({activeFilter: ""});   // Send empty string if no data found in file
            return;
        }
        res.send(data);
    });
...
router.put('/activeFilter',  function(req, res) {
    var body = req.body;
